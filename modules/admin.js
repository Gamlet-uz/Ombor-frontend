...
export async function renderAdmin(container, user, onLogout) {
  ...
  container.innerHTML = `
    <div class="bg-gray-50 min-h-screen flex flex-col pb-20">
      ...
      <!-- Меню (Таблар) -->
      <div class="flex bg-white border-b border-gray-200 shadow-sm">
      ...
      <div id="adminContent" class="p-4 transition-opacity duration-300"></div>
    </div>
  `;
  ...
  async function loadTab() {
    adminContent.innerHTML = `<div class="text-center py-10"><div class="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto"></div><p class="text-xs text-gray-400 mt-3 font-bold">Юкланмоқда...</p></div>`;
  ...
  function renderBugun() {
    const inputsHtml = catKeys.map(k => `
      <div class="flex items-center justify-between bg-white p-3.5 rounded-2xl shadow-sm border border-gray-100 mb-3">
    ...
    adminContent.innerHTML = `
      <h4 class="font-extrabold text-gray-800 mb-4 px-1 text-sm uppercase tracking-wider">Бугунги тушумни киритинг</h4>
      ${inputsHtml}
      
      <!-- Қора экран: Умумий Соф фойда -->
      <div class="bg-slate-900 text-white rounded-3xl p-6 shadow-xl mt-6 mb-5 text-center transition-all duration-300">
  ...
  function renderMonitoring() {
    if(monitoringData.length === 0) {
       adminContent.innerHTML = `<div class="text-center py-10 bg-white rounded-3xl border border-gray-100 shadow-sm"><p class="text-gray-400 font-bold text-sm">Тарих бўм-бўш</p></div>`; 
       return;
    }
    ...
    const cards = monitoringData.map(d => {
       ...
       const catsHtml = catKeys.map(k => {
          ...
          return `
            <div class="flex justify-between items-center text-xs py-3 border-b border-gray-100/50 last:border-0">
       ...
       return `
         <div class="bg-white rounded-3xl shadow-sm border border-gray-100 mb-4 overflow-hidden transition-all duration-200">
            <!-- Карточка сарлавҳаси (Босилганда очилади) -->
            <div class="p-4 cursor-pointer flex justify-between items-center bg-gray-50/50 hover:bg-gray-100 active:bg-gray-200 transition" onclick="this.nextElementSibling.classList.toggle('hidden')">
              <div>
            ...
            <div class="hidden p-4 border-t border-gray-100 bg-white">
              ...
              <!-- Кунлик якун -->
              <div class="mt-3 pt-3 border-t-2 border-dashed border-gray-200 flex justify-between text-xs font-bold bg-slate-50 p-3 rounded-xl">
    ...
    adminContent.innerHTML = `<h4 class="font-extrabold text-gray-400 mb-4 px-1 text-xs uppercase tracking-widest">Мониторинг Тарихи</h4>${cards}`;
  }
